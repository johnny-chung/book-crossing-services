...
public class OrderCancelledMongoEventHandler {
	...
	public OrderCancelledMongoEventHandler(BookMongoRepository bookRepo, LanguagesMongoRepository languageRepo, CategoriesMongoRepository categoryRepo) {
	...
	public void handle(@Payload OrderCancelledEvent orderCancelledEvent) {
		...
		LOGGER.info("Book service receive order cancelled event: " + orderCancelledEvent.getId());

		// check if book exist
		Optional<BookMongoEntity> existingBook = bookRepo.findById(
				orderCancelledEvent.getPostRef().getBookRef().getId());

		if (existingBook.isEmpty()) {
			LOGGER.error("Book {} not found", orderCancelledEvent.getPostRef().getBookRef().getId());
			throw new RetryableException("Book " + orderCancelledEvent.getPostRef().getBookRef().getId() + " not found");
		}
